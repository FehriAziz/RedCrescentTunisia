...

{
    ...
    public IActionResult ShippAll(int donnerId)
    {
        ...
        List<Donation> userDonnations = _context.Donations
            .Where(donation => donation.UserId == donnerId && donation.status == StaticData.Status.Valid && donation.Shipment.ShipStatus != StaticData.ShipStatus.Received)
            .Include(donation => donation.Donner)
            .Include(donation => donation.Shipment)
            .ToList();

        User? Donator = _context.Users.FirstOrDefault(user => user.UserId == donnerId);

        DonationandUser vl = new DonationandUser();
        vl.Alldonations = userDonnations;
        vl.User = Donator;
